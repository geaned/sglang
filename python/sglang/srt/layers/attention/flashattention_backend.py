...
def _should_disable_scheduler_metadata_precompute() -> bool:
    # Parallel *config* leaves (not live-shadowed topology sizes), so this
    # answers wherever the process has published.
...
class FlashAttentionBackend(AttentionBackend):
    ...
    def __init__(
        ...
        self.num_splits = (
            1
            if get_exec().deterministic.enable_deterministic_inference or fa4_no_splitkv
            else 0
        )
